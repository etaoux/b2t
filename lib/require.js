...
function extRequire(request, hostModule) {
	...
	var extRequest = './' + path.join(path.dirname(hostModule.filename), request).slice(root.length + 1).replace(/\\/g, '\/');
...
function patchRequire() {
	...
	require.extensions['.js'] = function(module, filename) {
		// 所有模块都可以直接require到b2t，主要为扩展提供支持
		module.paths.push(root);
		orig.apply(this, arguments);
	};
}

var extensions = require('./b2t').extensions;
// 保存所有的扩展模块
var exts = [];
patchRequire();
